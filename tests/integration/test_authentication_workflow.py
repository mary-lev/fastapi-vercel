...
class TestAuthenticationWorkflow:
    ...
    def setup_auth_test_data(self, test_db):
        ...
        student_user = User(
            internal_user_id="student-123",
            hashed_sub="student_hash",
            username="student",
            status="student"
        )
        
        professor_user = User(
            internal_user_id="professor-123",
            hashed_sub="professor_hash",
            username="professor",
            status="professor"
        )
        
        admin_user = User(
            internal_user_id="admin-123",
            hashed_sub="admin_hash",
            username="admin",
            status="admin"
        )
        ...
        course = Course(
            title="Test Course",
            description="Authentication test course",
    ...
    def test_enrollment_workflow(self, client, setup_auth_test_data, test_db):
        ...
        enrollment = test_db.query(CourseEnrollment).filter(
    ...
    def test_concurrent_enrollment_handling(self, client, setup_auth_test_data, test_db):
        ...
        enrollments = test_db.query(CourseEnrollment).filter(
            CourseEnrollment.user_id == student.id,
            CourseEnrollment.course_id == course.id
        ).all()

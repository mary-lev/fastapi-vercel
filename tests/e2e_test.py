...
class APITester:
    ...
    def test_course_endpoints(self):
        ...
        result = self.test_endpoint("GET", "/api/v1/courses/", 
                                   test_name="Get all courses")
        
        if result["success"] and result["data"]:
            course_id = result["data"][0]["id"] if result["data"] else 1
            self.test_endpoint("GET", f"/api/v1/courses/{course_id}",
                             test_name=f"Get course {course_id}")
            self.test_endpoint("GET", f"/api/v1/courses/{course_id}/lessons/",
                             test_name=f"Get lessons for course {course_id}")
    ...
    def test_submit_endpoints(self):
        ...
        result = self.test_endpoint(
            ...
            json_data={
                "user_answer": "A variable is a container for storing data",
                "task_id": 1
            },
    ...
    def test_user_endpoints(self):
        ...
        self.test_endpoint(
            "GET",
            f"/api/v1/students/{TEST_USER_ID}/solutions",
            test_name="Get user solutions"
        )
        
        self.test_endpoint(
            "GET",
            f"/api/v1/students/{TEST_USER_ID}/profile",
            test_name="Get user profile"
        )
        
        self.test_endpoint(
            "GET",
            f"/api/v1/students/{TEST_USER_ID}/courses/1/progress",
            test_name="Get user course progress"
        )
    ...
    def test_performance(self):
        ...
        if elapsed < 5:
            self.log_test("Response time check", True, 
                         f"Responded in {elapsed:.2f}s")
        else:
            self.log_test("Response time check", False, 

    ...
    def __init__(
        ...
        if REDIS_AVAILABLE and redis_url and strategy != CacheStrategy.DISABLED:
            try:
                ...
                logger.info("Redis cache initialized", category=LogCategory.SYSTEM, extra={"redis_url": redis_url})
            except Exception as e:
                logger.warning(
                    "Failed to connect to Redis, using memory cache only", category=LogCategory.SYSTEM, exception=e
                )
        else:
            logger.info(
    ...
    def get(self, key: str, layer: Optional[CacheLayer] = None) -> Optional[Any]:
        ...
        if layer in [None, CacheLayer.REDIS] and self.redis_client:
            try:
                ...
                logger.warning(f"Redis get failed for key: {key}", category=LogCategory.SYSTEM, exception=e)

        logger.debug(f"Cache miss: {key}", category=LogCategory.PERFORMANCE, extra={"key": key})
        return None

    def set(self, key: str, value: Any, ttl: Optional[int] = None, layer: Optional[CacheLayer] = None):
        """Set value in cache"""
        if self.strategy == CacheStrategy.DISABLED:
            return
        ...
        if layer in [None, CacheLayer.REDIS] and self.redis_client:
            try:
                ...
                logger.warning(f"Redis set failed for key: {key}", category=LogCategory.SYSTEM, exception=e)
    ...
    def delete(self, key: str):
        ...
        if self.redis_client:
            try:
                ...
                logger.warning(f"Redis delete failed for key: {key}", category=LogCategory.SYSTEM, exception=e)

        if deleted:
            logger.debug(f"Cache invalidated: {key}", category=LogCategory.PERFORMANCE, extra={"key": key})
    ...
    def invalidate_pattern(self, pattern: str):
        ...
        if self.redis_client:
            try:
                ...
                logger.warning(f"Redis pattern delete failed for: {pattern}", category=LogCategory.SYSTEM, exception=e)
    ...
    async def aget(self, key: str, layer: Optional[CacheLayer] = None) -> Optional[Any]:
        ...
        if layer in [None, CacheLayer.REDIS] and self.redis_async_client:
            try:
                ...
                logger.warning(f"Async Redis get failed for key: {key}", category=LogCategory.SYSTEM, exception=e)

        return None

    async def aset(self, key: str, value: Any, ttl: Optional[int] = None, layer: Optional[CacheLayer] = None):
        ...
        if layer in [None, CacheLayer.REDIS] and self.redis_async_client:
            try:
                ...
                logger.warning(f"Async Redis set failed for key: {key}", category=LogCategory.SYSTEM, exception=e)

    # ========================================================================
    # DECORATORS
    # ========================================================================

    def cache(self, cache_type: str = "default", ttl: Optional[int] = None, key_prefix: Optional[str] = None):
...
def api_cache(cache_type: str = "default", ttl: Optional[int] = None, key_builder: Optional[Callable] = None):
    """

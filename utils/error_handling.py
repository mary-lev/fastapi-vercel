...
def validate_user_permissions(user: Any, required_status: Optional[str] = None) -> None:
    ...
    if required_status and user.status != required_status:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail=f"Operation requires {required_status} privileges"
        )
...
def standardize_error_response(
    status_code: int, error_message: str, details: Optional[str] = None, request_id: Optional[str] = None
) -> HTTPException:
    """
    ...
    """
    content = {"success": False, "error": error_message, "status_code": status_code}
...
def handle_validation_errors(errors: list) -> HTTPException:
    ...
    return HTTPException(
        ...
        detail={"success": False, "error": "Validation Error", "errors": formatted_errors, "status_code": 422},
    )

...
async def get_student_form(
    ...
    try:
        ...
        if not submission:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Student form submission not found"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve student form"
        )


@router.get("/api/student-forms", response_model=Dict[str, Any])
async def get_all_student_forms(
    limit: int = 100,
    offset: int = 0,
    db: Session = Depends(get_db)
):
    ...
    """
    try:
        query = db.query(StudentFormSubmission).order_by(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve student forms"
        )
